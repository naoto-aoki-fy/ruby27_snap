...
CORE_INFO=$(../snap/snap_download.sh core20 stable "$ARCH" "$DOWNLOAD_DIR")
CORE_ASSERT=$(echo "$CORE_INFO" | grep '^ASSERT=' | cut -d= -f2)
CORE_SNAP=$(echo "$CORE_INFO" | grep '^SNAP=' | cut -d= -f2)
CORE_RESULT=$(../snap/snap_install.sh "$CORE_ASSERT" "$CORE_SNAP" /snap)
...
RUBY_INFO=$(../snap/snap_download.sh ruby 2.7/stable "$ARCH" "$DOWNLOAD_DIR")
RUBY_ASSERT=$(echo "$RUBY_INFO" | grep '^ASSERT=' | cut -d= -f2)
RUBY_SNAP=$(echo "$RUBY_INFO" | grep '^SNAP=' | cut -d= -f2)
RUBY_RESULT=$(../snap/snap_install.sh "$RUBY_ASSERT" "$RUBY_SNAP" /snap)
